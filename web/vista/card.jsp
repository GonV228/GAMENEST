...
<head>
	...
	<link rel="stylesheet" href="../css/card.css">
        ...
        %>
        <!--
            <div class="main-content" style="padding-top: 8%;">
             <div class="sidebar" style="padding-top: 5%;">
            <ul>
...
<body>
    <form action="<%=request.getContextPath()%>/ControladorProcesarCompra" method="post" enctype="multipart/form-data">
        <div class="container">
            <div class="left">
                <p>Métodos de Pago</p>
                <hr style="border:1px solid #ccc; margin: 0 15px;">
                <div class="methods">
                    <div onclick="doFun()" id="tColorA" style="color: #2C2C3E;"><i class="fa-solid fa-credit-card" style="color: #2C2C3E;"></i> Tarjeta de crédito</div>
                    <div onclick="doFunA()" id="tColorB" style="color: #2C2C3E;"><i class="fa-solid fa-building-columns"></i> Yape</div>
                    <div onclick="doFunB()" id="tColorC" style="color: #2C2C3E;"><i class="fa-solid fa-wallet"></i> Plin</div>
                </div>
                <hr style="border:1px solid #ccc; margin: 0 15px;">
            </div>
            <div class="center">
                <a href=""><img alt="Credit Card Logos" title="Logos" src="https://www.shift4shop.com/images/credit-card-logos/cc-lg-4.png" width="250" height="auto"/></a>
                <hr style="border:1px solid #ccc; margin: 0 15px;">
                ...
                <div class="card-details">
                    ...
                    <div class="email">
                        <p>Nombres</p>
                        <input type="nombres" name="nombres" id="nombres" value="<%=nombre%>" <% if (usuarioAutenticado) { %>readonly<% }%> style="margin-bottom: 20px;"required>
       ...
       <script> //Obtener detalles del carrito
            ...
            function displayOrderDetails() {
                ...
                if (cart.length === 0) {
                    ...
                    for (let i = 0; i < cart.length; i++) {
                        ...
                        total += subtotal;
                        orderDetails.innerHTML +=
                            '<p>' + cart[i].quantity + ' x ' + cart[i].nombre + ' - $' + cart[i].precio + '</p>';
                    }
            ...
            function updateTotalInForm() {
                ...
                let total = 0;
                for (let i = 0; i < cart.length; i++) {
                    total += cart[i].precio * cart[i].quantity;
                }
                document.getElementById('total').value = total;
            }
           ...
           document.getElementById('payButton').addEventListener('click', function() {
            ...
            xhr.open('POST', '<%=request.getContextPath()%>/procesarCompra', true);
            xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
            xhr.send('total=' + total + '&email=' + encodeURIComponent(email));
            xhr.onload = function() {
                if (xhr.status >= 200 && xhr.status < 400) {
                    // Redirige al usuario a una página de confirmación o muestra un mensaje
                    window.location.href = '<%=request.getContextPath()%>/vista/confirmacion.jsp';
                } else {
                    // Manejo de errores
